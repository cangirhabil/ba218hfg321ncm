...
source ../venv/bin/activate
cd ../fuzzer
NODE_OPTIONS="--max-old-space-size=8192" python fuzzer.py --hour 24 --minute 0 --url $TARGET_URL --name ${project_name} --only-checker y --roles Admin User Editor Viewer Anonymous --config ../configs/config-dolibarr.yaml 2>&1 | tee ../"${project_name}-CHECK-$(hostname)-${timestamp}.log"
...
echo "FUZZING COMPLETED FOR DOLIBARR"

## USER CREDENTIALS:
## admin: admin123
## testuser: TestUser2024!
## editor: EditorPass2024!
## viewer: ViewerPass2024!
## admin_test: AdminTest2024!