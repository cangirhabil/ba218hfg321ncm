...
class HTTPRequest:
    ...
    def update_param_from_paramvals(self, add_paramvals=None, is_drop_previous_paramvals=True):
        ...
        print(f"[HTTPREQUEST {self.role}] Update url and body param from paramvals:")
        url_param = ""
        body_param = ""
            ...
            if paramval.position==ParamValuePosition.URL:
                ...
                if paramval.is_nested():
                    ...
                    if paramval.param==None or paramval.param=="None":
                        print(f"[HTTPREQUEST {self.role}] Skip None Paramval to be included")
                    else:
        ...
        print(f"[HTTPREQUEST {self.role}] New post_data_encoded: ",self.post_data_encoded)
        print(f"[HTTPREQUEST {self.role}] New param encoded: ",self.param_encoded)
    ...
    def extract_param_value(self):
        self.paramvals = list()
        self.paramvals_without_nonce = list()
        self.num_sysgen = 0
        if self.post_data_json:
            for key in self.post_data_json.keys():
                ...
                if isinstance(self.post_data_json[key],str):
                    ...
                    print(f"[HTTPREQUEST {self.role}] {key} --> {self.post_data_json[key]} is not string. No nested paramval check")
