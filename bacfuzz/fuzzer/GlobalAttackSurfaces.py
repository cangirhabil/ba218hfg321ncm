...
from general_functions import extract_sql_command_and_table
...
class GlobalAttackSurfaces:
    ...
    def analyse_and_print_final_result2(self, start_time, is_finish=False):
        # responses = list()
        endpoints = list()
        stats = dict()
        stats['pro_broken'] = 0
        ...
        broken_objects = set()
        broken_properties = set()
        broken_functions = set()

        vulnerable_elements = set()

        print(f"[GLOBALATTACKSURFACE] Saved in {filename}")
        with open(filename, "w") as txt_file:
            whole_corpus = list()
            for attack_target in global_attack_surfaces.data:
                whole_corpus += attack_target.corpus

            sorted_corpus = sorted(whole_corpus, key = lambda x: (self.get_sent_role(x), x.request.full_url, self.get_mutated_params(x), self.get_numerical_source(x), self.get_detected_time(x)))
            current_label = VerificationLabel.OBJECT_BROKEN
            ...
            for inp in sorted_corpus:
                if inp.label==VerificationLabel.FUNCTIONAL_BROKEN:
                    stats['func_broken'] += 1
                    broken_functions.add(inp.request.url)
                elif inp.label==VerificationLabel.OBJECT_BROKEN:
                    stats['obj_broken'] += 1
                    for pv in inp.mutated_paramvals:
                        broken_objects.add(pv.param)
                elif inp.label==VerificationLabel.PROPERTY_BROKEN:
                    stats['pro_broken'] += 1
                    for pv in inp.mutated_paramvals:
                        broken_properties.add(pv.param)
                elif inp.label==VerificationLabel.ERROR:
                    stats['error'] += 1
                    continue
                else:
                    continue

                val = inp.mutated_paramvals[0].value if len(inp.mutated_paramvals)>0 else "0"
                if inp.label==VerificationLabel.FUNCTIONAL_BROKEN and broken_url!=inp.request.url:
                    broken_url = inp.request.url
                elif mutated_val==val and sent_role==inp.sent_role:
                    continue
                else:
                    mutated_val = val
                    sent_role = inp.sent_role

                if inp.is_verification_proof and verification_proof_num<10:
                    verification_proof_num += 1
                    txt_file.write("**")
                    is_verification_proof = True
                elif inp.request in endpoints:
                    continue
                else:
                    endpoints.append(inp.request)
                    txt_file.write("\n---\n")
                    verification_proof_num = 0
