...
from playwright.sync_api import sync_playwright, Request, Locator, expect, Route, APIResponse

from config import config
from function import get_form_id, cleanhtml, is_same_param_field, extract_paramvals_from_url, save_param_value_to_dict, \
    delete_files, get_query_keys
...
class MainDriver:
    ...
    async def find_and_click_buttons(self, page=None):
        ...
        for button in buttons:
            ...
            try:
                button_name = await button.inner_text(timeout=2000)
                if button_name=="Delete" or button_name=="View" or button_name=="Edit" or button_name=="Preview":
                    button_name += str(page.url)
    ...
    async def find_and_fill_forms(self, page=None):
        ...
        try:
            ...
            for form in forms:
                ...
                if form_name==None or form_name=="":
    ...
    def payload_analyze(self, request):
        if request.num_sysgen>0:
            print(f"[MAINDRIVER {self.role}] Calling payload_analyze")
            prompt = build_param_filtering_prompt(request)
            try:
                ...
                if response:
                    ## We assume the response is something like "action, closed, closedpostboxesnonce, page"
                    reference_params = response.strip().split(",")
                    request.reference_params = [x.strip() for x in reference_params]

                    for param in request.paramvals:
                        for param_nested in param.paramvals:
                            if param_nested.param in request.reference_params:
                                if param_nested.param!="" and param_nested.is_system_param:
                                    param_nested.is_reference = True
                        if param.param in request.reference_params:
                            if param.param!="" and param.is_system_param:
                                param.is_reference = True

                    print(f"[MAINDRIVER {self.role}] LLM has analyzed this request payload:",request)
            except Exception as e:
                print(e)
                ...
                if self.is_avoided_links("TXT",request.url):
                    ...
                    if not self.is_back_request and self.is_avoided_action(req):
                        ...
                        if request.is_navigation_request():
                            if self.is_crawling_complete == False and not self.is_back_request:
                                await self.mock_route(route)
                                print(f"[MAINDRIVER {self.role}] Getting a navigation request to {request.url} while crawling is incomplete")
                                self.save_link(request.url)
                                print(f"[MAINDRIVER {self.role}] Trying to back to {self.current_url}")
                                self.is_back_request = True
                                await self.page.goto(self.current_url)
                                await self.page.wait_for_load_state('domcontentloaded')
                            else:
                    ...
                    print(f"[MAINDRIVER {self.role}] ----END OF INTERCEPTION {request.url}----")
        else:
            await route.continue_()
    ...
    def save_link(self, full_link):
        ...
        if is_new_link:
            formatted_link = {}
            formatted_link['text'] = "LinkTriggeredFromAction"
            ...
            print(f"[MAINDRIVER {self.role}] A new link from req interception is found and added: {full_link}")
    ...
    def save_captured_id(self,start_time):
        filename = f"[ID]{self.role}_{start_time.hour}{start_time.minute}{start_time.second}{start_time.microsecond}"
        print(f"[MAINDRIVER {self.role}] Storing the captured ID to: ../result/{filename}.txt")
        with open(f"../result/{filename}.txt", "w") as txt_file:
            i = 0
            for id in dictionary.ids[self.role]:
                i += 1
                txt_file.write(f"{i}. {id} ==> {dictionary.ids[self.role][id]}\n")

    def save_captured_paramvals(self, start_time):
        filename2 = f"[DICTPARAMVALS]{self.role}_{start_time.hour}{start_time.minute}{start_time.second}{start_time.microsecond}"
    ...
    def print_captured_request(self):
        print(f"[MAINDRIVER {self.role}] ------------------------------------------------")
        print(f"[MAINDRIVER {self.role}] ----------PRINTING CAPTURED REQUESTS------------")
        print(f"[MAINDRIVER {self.role}] ------------------------------------------------")
        i = 0
        for request in self.corpus:
            i += 1
            print(f"[MAINDRIVER {self.role}] {i}. {request}")
        print(f"[MAINDRIVER {self.role}] ------------------------------------------------")
    ...
    async def crawl_new_page(self, page):
        ...
        if opener:
            print(f"[MAINDRIVER {self.role}] {opener.url} OPENS A NEW TAB! CRAWLING THE NEW PAGE: {page.url}")
        else:
            print(f"[MAINDRIVER {self.role}] OPENING A NEW TAB! CRAWLING THE NEW PAGE: {page.url}")
        ...
        print(f"[MAINDRIVER {self.role}] The Tab of: {page.url} is going to be closed")
        ## Close the page tab
        await page.close()
    ...
    async def crawl(self, homepage=None):
        ...
        self.page.on("popup", lambda page: print(f"[MAINDRIVER {self.role}] New Popup page: {page}"))
...
## ---
    async def handle_forms(self, page):
        ...
        for form in forms:
            ...
            for input_elem in inputs:
                ...
                if input_type in ['text', 'email', 'search', 'url', 'tel', 'password'] or not input_type:
        ...
        for link in links:
            ...
            if abs_url not in self.visited:
                try:
                    ...
                    await page.wait_for_timeout(500)  # Allow for navigation
                except:
                    pass
