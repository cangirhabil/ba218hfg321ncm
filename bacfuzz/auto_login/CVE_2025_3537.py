...
def run_admin(playwright: Playwright, config) -> None:
    ...
    page.get_by_placeholder("Email").fill("admin@local.co")
...
def run_2(playwright: Playwright, config) -> None:
    ...
    page.goto("http://localhost:8088/")
    page.get_by_placeholder("Email").click()
    page.get_by_placeholder("Email").fill("employee@local.co")
    page.get_by_placeholder("Password").click()
    page.get_by_placeholder("Password").fill("employee123")
    page.get_by_placeholder("Password").press("Enter")
    
    page.wait_for_timeout(3000);

    config.homepages['Employee'] = page.url
    page.context.storage_state(path=f"{folder_name}/Employee.json")
    # ---------------------
    context.close()
    browser.close()
